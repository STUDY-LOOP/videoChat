  ...
  connectDB.query(
    ...
    function (error, results, fields) {
      ...
      if (results.length == 0) {
        ...
        res.send(
          '<script type="text/javascript">alert("이미 존재하는 아이디입니다."); document.location.replace("/signup");</script>'
        );
        res.end();
      }
    }
...
app.post('/login', function (req, res) {
  ...
  if (username && password) {
    connectDB.query(
      'SELECT * FROM user WHERE u_id = ? AND password = ?',
      [username, password],
      function (error, results, fields) {
    ...
    res.send(
      '<script type="text/javascript">alert("username과 password를 입력하세요!"); document.location.href="/";</script>'
    );
    res.end();
  }
});
